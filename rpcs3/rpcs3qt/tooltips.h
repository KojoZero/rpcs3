...
public:
	...
	const struct settings
	{
		...
		const QString post_antialiasing_mode = tr("Lightweight post-process antialiasing.\nFXAA is extremely lightweight but slightly blurs more, SMAA is a bit heavier but higher quality.\nBecause these antialiasing filters are higher quality and lighter weight than the ones used on console, it's highly recommended to use game patches to disable in-game mlaa/fxaa.\nIt's also recommended to use FXAA/SMAA if using FidelityFX Super Resolution or NNEDI3");
		const QString output_scaling_mode = tr("Final image filtering. Nearest applies no filtering, Bilinear smooths the image, Lanczos smooths the image but is much sharper, FidelityFX Super Resolution enhances upscaled images and NNEDI3 enhances upscaled images with a small neural network.\nDue to NNEDI3 being a heavy upscaler (heavier than resolution scale), it's only recommended for 720p and when unable to increase resolution scale\nIf the game is rendering at an internal resolution lower than your window resolution, FidelityFX will handle the upscale.\nFidelityFX can cause visual artifacts.\nFidelityFX does not work with stereo 3D output for now.");
