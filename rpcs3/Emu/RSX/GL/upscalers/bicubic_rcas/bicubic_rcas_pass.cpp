#include "bicubic_rcas_pass.h"
#include "bicubic_rcas_shader.h"

namespace gl
{
	bicubic_rcas_pass::bicubic_rcas_pass()
	{
		m_vertices = {
		...
		// Compile BICUBIC_RCAS Shaders
		std::array<std::string, 3> BICUBIC_RCAS_PASS_X_VERT_DATA;
		std::array<std::string, 3> BICUBIC_RCAS_PASS_X_FRAG_DATA;
		BICUBIC_RCAS_PASS_X_VERT_DATA[0] = CATMULL_ROM_PASS_0_VERT;
		BICUBIC_RCAS_PASS_X_FRAG_DATA[0] = CATMULL_ROM_PASS_0_FRAG;
		BICUBIC_RCAS_PASS_X_VERT_DATA[1] = CATMULL_ROM_PASS_1_VERT;
		BICUBIC_RCAS_PASS_X_FRAG_DATA[1] = CATMULL_ROM_PASS_1_FRAG;
		BICUBIC_RCAS_PASS_X_VERT_DATA[2] = FSR_PASS_1_VERT;
		BICUBIC_RCAS_PASS_X_FRAG_DATA[2] = FSR_PASS_1_FRAG;
		replaceInclude(BICUBIC_RCAS_PASS_X_FRAG_DATA[2], "ffx_a.h", FFX_A);
		replaceInclude(BICUBIC_RCAS_PASS_X_FRAG_DATA[2], "ffx_fsr1.h", FFX_FSR1);

		for (size_t i = 0; i < m_vert_shader.size(); i++)
		{
			m_vert_shader[i].create(::glsl::program_domain::glsl_vertex_program, BICUBIC_RCAS_PASS_X_VERT_DATA[i]);
			m_vert_shader[i].compile();
			m_frag_shader[i].create(::glsl::program_domain::glsl_fragment_program, BICUBIC_RCAS_PASS_X_FRAG_DATA[i]);
		...
		printf("Allocated BICUBIC_RCAS Textures\n");
	...
	bicubic_rcas_pass::~bicubic_rcas_pass()
	{
		printf("Destroying: BICUBIC_RCAS PASS\n");
		m_vao.remove();
		m_vbo.remove();
		for (size_t i = 0; i < m_vert_shader.size(); i++)
		{
		...
		printf("Destroyed: BICUBIC_RCAS PASS\n");
	}

	void bicubic_rcas_pass::attachUniforms(GLuint shader_program_id)
	{
	...
	void bicubic_rcas_pass::allocateTextures(areai internal_res, areai screen_res)
	{
	...
	void bicubic_rcas_pass::replaceInclude(std::string& shader_source, std::string include_name,
	...
	void bicubic_rcas_pass::reset_sampler_states()
	{
	...
	gl::texture* bicubic_rcas_pass::scale_output(
