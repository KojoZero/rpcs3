...
namespace gl
{
	class nnedi3_pass : public upscaler
	{
	...
	private:
		...
		gl::fbo m_fbo;
		gl::fbo m_flip_fbo;
		std::array<std::unique_ptr<saved_sampler_state>, 2> saved_sampler_states;
		std::array<gl::sampler_state, 2> m_sampler;
		std::array<std::unique_ptr<gl::texture>, 9> m_intermediate_texture;
		std::array<gl::glsl::shader, 9> m_vert_shader;
		std::array<gl::glsl::shader, 9> m_frag_shader;
		std::array<gl::glsl::program, 9> m_program;
